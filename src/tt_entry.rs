...
impl TTEntry {
    pub const fn empty() -> TTEntry {
        TTEntry {hash: EMPTY_HASH, score: 0, e_type: EntryType::Alpha, depth: 0, best: NULL_MOVE}
    }

    fn entry_get_score(&self, hash: u64, alpha: i32, beta: i32, depth: usize, ply: i32) -> Option<i32> {
        if self.hash != hash || (self.depth as usize) < depth  { return None; }
    ...
    fn entry_get_bestmove(&self, hash: u64) -> Option<Move> {
        if self.hash == hash && self.best != NULL_MOVE { Some(self.best) } else { None }
    }

    fn entry_update (
        &mut self, hash: u64, score: i32, e_type: EntryType, depth: usize, best: Option<Move>, ply: usize
    ) {
        if self.hash == hash && (self.depth as usize) > depth { return; }
...
impl Entry for Cell<TTEntry> {
    ...
    fn clear(&self) { self.set(TTEntry::empty()) }
}
