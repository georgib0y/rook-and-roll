use crate::board::{BKS_STATE, Board, BQS_STATE, ROOK, WKS_STATE, WQS_STATE};
...
impl BoardBuilder {
    ...
    pub fn apply_promo_cap(&mut self, move_type: MoveType, piece: usize, xpiece: usize, to: usize) {
        ...
        self.mg_value += -PST::mid_pst(xpiece, to) - PST::mid_pst(piece, to) + PST::mid_pst(promo_piece, to);
        self.eg_value += MAT_SCORES[promo_piece] - MAT_SCORES[xpiece] - MAT_SCORES[piece];
        self.eg_value += -PST::end_pst(xpiece, to) - PST::end_pst(piece, to) + PST::end_pst(promo_piece, to);
        self.halfmove = 0;
    }
...
pub fn copy_castle_state(mut castle_state: u8, piece: usize, from: usize, to: usize, hash: &mut u64) -> u8 {
