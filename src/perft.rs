...
use crate::new_movegen::MoveList;

pub fn perft_new_movegen(board: &Board, depth: usize) -> usize {
    if depth == 0 {
        return 1;
    }
    ...
    // let check = false;
    let move_list = MoveList::all(board, false);
    println!("{board}");
    for m in move_list {

        let b = board.copy_make(&m);
        println!("{b}{m}\n");
        if (moved_into_check(&b, &m)) || !is_legal_move(&b, &m) {
            continue;
        }
        mc += perft_new_movegen(&b, depth - 1);
    }
